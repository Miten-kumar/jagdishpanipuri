...
function FoodPlaceholder({ name }: { name: string }) {
  ...
  return (
    ...
    >
      <span className="text-5xl font-serif font-bold text-amber-400/40">
        {name[0]}
      </span>
    </div>
  );
...
export default function MenuPage() {
  ...
  const canPlaceOrder = siteContent?.isOrderingEnabled ?? false;
        ...
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          >
            ...
            <p className="text-muted-foreground text-lg max-w-xl mx-auto">
              Explore our carefully crafted menu — from morning coffee to
              evening bites.
            </p>
          </motion.div>
        </div>
    ...
    >
      <div className="h-44 overflow-hidden relative">
        {item.imageUrl ? (
          ...
          <div
            className={`w-full h-full bg-amber-50 flex items-center justify-center`}
          >
            <span className="text-5xl font-serif font-bold text-amber-400/40">
              {item.name[0]}
            </span>
      ...
      <div className="p-4">
        ...
        >
          <Plus className="w-3.5 h-3.5" />
           {canPlaceOrder ? "Add to Order" : "Ordering Disabled"}
        </Button>
      </div>
    </motion.div>
  );
