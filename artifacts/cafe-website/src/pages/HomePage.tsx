...
function FoodPlaceholder({ name }: { name: string }) {
  ...
  return (
    <div
      className={`w-full h-full ${colors[idx]} flex items-center justify-center`}
    >
      <span className="text-4xl font-serif font-bold text-amber-400/50">
        {name[0]}
      </span>
    </div>
  );
...
export default function HomePage() {
  ...
  const heroSubtitle =
    siteContent?.heroSubtitle ?? "Fresh, bold flavors crafted with love.";
  const heroImageUrl = siteContent?.heroImageUrl;
  const aboutTitle = siteContent?.aboutTitle ?? "Our Story";
  const aboutText =
    siteContent?.aboutText ??
    "Urban Bites was born from a passion for honest food and great coffee.";
  const aboutImageUrl = siteContent?.aboutImageUrl;

  const featuredItems =
    menuItems?.filter((i) => i.isFeatured).slice(0, 6) ?? [];
        ...
        {heroImageUrl ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-amber-900 via-orange-800 to-red-900">
            <div
              ...
              style={{
                backgroundImage:
                  "radial-gradient(circle at 25% 25%, rgba(255,200,50,0.4) 0%, transparent 50%), radial-gradient(circle at 75% 75%, rgba(220,50,50,0.3) 0%, transparent 50%)",
              }}
            />
          </div>
        )}
          ...
          <div className="max-w-7xl mx-auto px-4">
                ...
                >
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span className="text-primary font-bold text-lg ml-2 shrink-0">
                        {currencySymbol}
                        {item.price}
                      </span>
                    </div>
            ...
            <div className="text-center mt-10">
              <Button variant="outline" size="lg" asChild>
                <Link href="/menu" data-testid="button-full-menu">
                ...
                {aboutImageUrl ? (
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-amber-100 to-orange-200 flex items-center justify-center">
                    <div className="text-center text-amber-600">
                      ...
                      <p className="font-serif text-2xl font-bold opacity-40">
                        Our Kitchen
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                      <span className="text-2xl">
                        {cat.iconName.length === 1 ? cat.iconName : "🍽"}
                      </span>
                    </div>
        ...
        <div className="max-w-4xl mx-auto text-center px-4">
          ...
          >
            ...
            <p className="text-primary-foreground/80 text-lg mb-8 max-w-xl mx-auto">
              Visit us today or make a reservation for your next special
              occasion.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              >
                <Link href="/menu" data-testid="button-menu-cta">
                  See Our Menu
                </Link>
              </Button>
            </div>
